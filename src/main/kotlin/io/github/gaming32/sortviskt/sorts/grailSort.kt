...
class GrailSortStaticOop : AbstractSort("Grail Sort (Static OOP)") {
    override fun run(list: VisualList) {
        io.github.holygrailsortproject.rewrittengrailsort.GrailSort<Int>()
            .commonSort(list, arrayOfNulls(GRAIL_STATIC_EXT_BUFFER_LEN))
    }
}
...
class GrailSortDynamicOop : AbstractSort("Grail Sort (Dynamic OOP)") {
    override fun run(list: VisualList) {
        ...
        io.github.holygrailsortproject.rewrittengrailsort.GrailSort<Int>().commonSort(list, arrayOfNulls(bufferLen))
    }
}
