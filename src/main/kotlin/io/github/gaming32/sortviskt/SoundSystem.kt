...
class SoundSystem(private val window: MainWindow) : AutoCloseable {
    companion object {
        private const val PITCH_MIN = 25
        private const val PITCH_MAX = 105
        private const val SOUND_MUL = 1
    ...
    fun tick() {
        ...
        list.marksLock.withLock {
            for (mark in list.marks) {
                ...
                var vel = ((PITCH_MAX - pitchMajor).toDouble().pow(2) * 15.0.pow(-0.25) * 64 * SOUND_MUL).toInt() / 2
